<template>
    ...
    <div class="search-box">
      <Search v-if="index == 0"
        placeholder="书名搜索"
        v-model="bookParams.type_name"
        :autoFixed="false"
        @on-submit="search('book')"
      />
      <Search v-else
        placeholder="申报单位搜索"
        v-model="schoolParams.type_name"
        :autoFixed="false"
...
import LoadMore from 'components/loading-more';
	export default {
		data() {
			return {
        SubjectCountUrl:'/pmpheep/decPosition/list/bookList',   //书名统计URL
        contentCountUrl:'/pmpheep/decPosition/list/schoolListChosen'   ,  //学校统计URL
        subjectParams: {
          type_name:'',
          pageNumber:1,
          pageSize:20,
    ...
    created () {
      this.subjectParams.product_type = this.$route.params.product_type;
      this.contentParams.product_type = this.$route.params.product_type;
      this.getBookTableData();
    },
    methods: {
      /* 获取按书名统计申报情况 */
      getSubjectData(flag){
        this.$axios.get(this.SubjectCountUrl,{
